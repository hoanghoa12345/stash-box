...
import { type Post, PostType, PostUpdate } from "./types.ts";
import { MetaTagService } from "../metatag/metatag.ts";
import { stringToSlug } from "../../utils/helpers.ts";
import type { QueryArguments } from "../../config/deps.ts";
...
export class PostService {
  ...
  public static async show(id: string) {
    const query = "SELECT * FROM sb_posts WHERE id = $1 AND deleted_at IS NULL";
    const params = [id];
    const result = await db.query(query, params);
